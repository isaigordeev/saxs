...
class TestThresholdCondition:
    ...
    def test_threshold_condition_evaluate_missing_key(self) -> None:
        ...
        sample_no_key = SampleMetadata({"other_field": "value"})
    ...
    def test_threshold_condition_evaluate_with_different_numeric_types(
        ...
        sample_int = SampleMetadata({"value": 15})
        assert condition.evaluate(sample_int) is True

        sample_int_below = SampleMetadata({"value": 5})
        assert condition.evaluate(sample_int_below) is False

        # Test with float values
        sample_float = SampleMetadata({"value": 15.5})
        assert condition.evaluate(sample_float) is True

        sample_float_below = SampleMetadata({"value": 5.5})
    ...
    def test_threshold_condition_evaluate_with_non_numeric_values(
        ...
        sample_string = SampleMetadata({"value": "15"})
        ...
        sample_bool = SampleMetadata({"value": True})
        try:
    ...
    def test_threshold_condition_evaluate_with_very_small_values(self) -> None:
        ...
        sample_above = SampleMetadata({"value": 2e-6})
        assert condition.evaluate(sample_above) is True

        # Test with value below small threshold
        sample_below = SampleMetadata({"value": 5e-7})
        assert condition.evaluate(sample_below) is False
    ...
    def test_threshold_condition_evaluate_float_precision(self) -> None:
        ...
        sample_precise = SampleMetadata({"value": 0.1000000001})
    ...
    def test_threshold_condition_evaluate_with_none_value(self) -> None:
        ...
        sample_none = SampleMetadata({"value": None})
    ...
    def test_threshold_condition_multiple_conditions(self) -> None:
        ...
        sample_all_pass = SampleMetadata(
        ...
        sample_partial_pass = SampleMetadata(
            {
