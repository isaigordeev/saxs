...
def find_peaks_stage(mock_chaining_stage, mock_condition):
    ...
    return FindAllPeaksStage(chaining_stage=mock_chaining_stage, condition=mock_condition)


class TestFindAllPeaksStage:
    """Test cases for FindAllPeaksStage class."""

    def test_find_peaks_stage_creation(self, mock_chaining_stage, mock_condition):
        """Test creating FindAllPeaksStage."""
        stage = FindAllPeaksStage(chaining_stage=mock_chaining_stage, condition=mock_condition)
        
        assert stage.chaining_stage == mock_chaining_stage
        assert stage.condition == mock_condition

    def test_find_peaks_stage_inheritance(self, mock_chaining_stage, mock_condition):
        """Test that FindAllPeaksStage inherits from AbstractConditionalStage."""
        stage = FindAllPeaksStage(chaining_stage=mock_chaining_stage, condition=mock_condition)
        from saxs.saxs.core.stage.abstract_stage import AbstractConditionalStage
        assert isinstance(stage, AbstractConditionalStage)

    @patch('saxs.saxs.processing.stage.peak.find_peak_stage.find_peaks')
    def test_find_peaks_stage_process_basic(self, mock_find_peaks, sample_data, find_peaks_stage):
        ...
        np.testing.assert_array_equal(call_args[1]["x"], sample_data.get_intensity_array())

    @patch('saxs.saxs.processing.stage.peak.find_peak_stage.find_peaks')
    def test_find_peaks_stage_process_peaks_detection(self, mock_find_peaks, sample_data, find_peaks_stage):
        """Test FindAllPeaksStage process method with peak detection."""
        # Mock find_peaks to return multiple peaks
        mock_find_peaks.return_value = (np.array([2, 4, 6]), {"peak_heights": [20.0, 30.0, 20.0]})
    ...
    @patch('saxs.saxs.processing.stage.peak.find_peak_stage.find_peaks')
    def test_find_peaks_stage_process_no_peaks(self, mock_find_peaks, sample_data, find_peaks_stage):
    ...
    @patch('saxs.saxs.processing.stage.peak.find_peak_stage.find_peaks')
    def test_find_peaks_stage_process_preserves_other_data(self, mock_find_peaks, sample_data, find_peaks_stage):
    ...
    @patch('saxs.saxs.processing.stage.peak.find_peak_stage.find_peaks')
    def test_find_peaks_stage_process_metadata_handling(self, mock_find_peaks, sample_data, find_peaks_stage):
        """Test FindAllPeaksStage metadata handling."""
        mock_find_peaks.return_value = (np.array([4]), {"peak_heights": [30.0]})
        
        result = find_peaks_stage.process(sample_data)
        
        # Should preserve original metadata and add peaks
        original_metadata = sample_data.get_metadata_dict()
        result_metadata = result.get_metadata_dict()
        
        assert "source" in result_metadata
        assert result_metadata["source"] == original_metadata["source"]
        assert "peaks" in result_metadata

    @patch('saxs.saxs.processing.stage.peak.find_peak_stage.find_peaks')
    def test_find_peaks_stage_process_with_different_peaks(self, mock_find_peaks, sample_data, find_peaks_stage):
        ...
        mock_find_peaks.return_value = (np.array([1, 4, 7]), {"peak_heights": [15.0, 30.0, 20.0]})
        result2 = find_peaks_stage.process(sample_data)
        assert len(result2.get_metadata_dict()["peaks"]) == 3

    @patch('saxs.saxs.processing.stage.peak.find_peak_stage.find_peaks')
    def test_find_peaks_stage_process_with_find_peaks_error(self, mock_find_peaks, sample_data, find_peaks_stage):
    ...
    def test_find_peaks_stage_process_with_minimal_data(self, find_peaks_stage):
        ...
        with patch('saxs.saxs.processing.stage.peak.find_peak_stage.find_peaks') as mock_find_peaks:
            mock_find_peaks.return_value = (np.array([]), {})
    ...
    def test_find_peaks_stage_process_immutability(self, sample_data, find_peaks_stage):
    ...
    def test_find_peaks_stage_process_consistency(self, sample_data, find_peaks_stage):
        ...
        with patch('saxs.saxs.processing.stage.peak.find_peak_stage.find_peaks') as mock_find_peaks:
            ...
            assert result1.get_metadata_dict()["peaks"].tolist() == result2.get_metadata_dict()["peaks"].tolist()

    def test_find_peaks_stage_process_with_none_error(self, find_peaks_stage):
        ...
        with patch('saxs.saxs.processing.stage.peak.find_peak_stage.find_peaks') as mock_find_peaks:
            mock_find_peaks.return_value = (np.array([1]), {"peak_heights": [8.0]})
    ...
    def test_find_peaks_stage_process_with_empty_data(self, find_peaks_stage):
    ...
    def test_find_peaks_stage_process_with_different_data_types(self, find_peaks_stage):
    ...
    def test_find_peaks_stage_process_with_negative_values(self, find_peaks_stage):
        ...
        with patch('saxs.saxs.processing.stage.peak.find_peak_stage.find_peaks') as mock_find_peaks:
            mock_find_peaks.return_value = (np.array([2]), {"peak_heights": [10.0]})
    ...
    def test_find_peaks_stage_process_with_nan_values(self, find_peaks_stage):
        """Test FindAllPeaksStage with NaN values."""
        q_values = np.array([0.1, 0.2, 0.3])
        ...
        with patch('saxs.saxs.processing.stage.peak.find_peak_stage.find_peaks') as mock_find_peaks:
            mock_find_peaks.return_value = (np.array([0]), {"peak_heights": [10.0]})
    ...
    def test_find_peaks_stage_process_with_inf_values(self, find_peaks_stage):
        ...
        with patch('saxs.saxs.processing.stage.peak.find_peak_stage.find_peaks') as mock_find_peaks:
            mock_find_peaks.return_value = (np.array([1]), {"peak_heights": [np.inf]})
    ...
    def test_find_peaks_stage_process_chain_calls(self, sample_data, find_peaks_stage):
