...
from saxs.algo.functions import gauss, parabole
...
class ParabolePeakKernel(ProminencePeakKernel):
    ...
    def gauss_peak_fitting_minimize(self, i):
        if len(self.peaks) > i:
            if np.size(self.peaks) != 0:
                delta = (
                    self.parabole_popt_for_gauss[self.peaks[i]][0]
                    / self.delta_q
                )
                ...
                def loss_function(params):
                    ...
                    y_pred = gauss(
                        ...
                        self.current_q_state[self.peaks[i]],
                        sigma,
                        ampl,
                    )
                ...
                result = minimize(
                    loss_function,
                    self.parabole_popt_for_gauss[self.peaks[i]],
                    method="BFGS",
                )
