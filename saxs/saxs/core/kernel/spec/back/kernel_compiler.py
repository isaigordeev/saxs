...
class StageCompiler:
    ...
    def get_parser(self, path_to_yaml: str = "kernel.yaml"):
        return DeclarativePipeline.from_yaml(path_to_yaml)

    def compile(self):
        parser: DeclarativePipeline = self.get_parser()

        print(parser)

        print(parser.policy_decl_specs)
        print(parser.stage_decl_specs)

        policy_decl_specs: Buffer[PolicyDeclSpec] = parser.policy_decl_specs
        stage_decl_specs: List[StageDeclSpec] = parser.stage_decl_specs

        compiler = PipelineSpecCompiler()

        policy_specs: Buffer[PolicySpec] = compiler._build_policy_specs(
            policy_decl_specs
        )
        stage_specs: Buffer[StageSpec] = compiler._build_stage_specs(
            stage_decl_specs
        )
