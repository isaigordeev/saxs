...
This document defines the official code style and naming conventions for the SAXS codebase. These rules are derived from the current codebase patterns and enforce consistency going forward.
...
5. [Python Features](#5-python-features)
6. [Comments and Documentation](#6-comments-and-documentation)
7. [Logging](#7-logging)
8. [Common Patterns](#8-common-patterns)
9. [Tools and Linting](#9-tools-and-linting)
...
**Rule:** Use `IAbstract*` prefix for abstract base classes and interfaces.
...
class IAbstractRequestingStage(IAbstractStage):
    """Abstract class for stages that can request other stages."""
...
**Exception:** Protocol classes (if used) should use `Protocol` suffix instead.
...
class TAbstractMetadata(Generic[TMetadataSchemaDict, TMetadataKeys]):
...
**Rationale:** The `T` prefix indicates this is a type-level construct used for type safety, not a regular class.
...
**Rationale:** Makes it clear this is a schema definition, not a runtime class.
...
|--------|---------|---------|
| `*Condition` | Boolean evaluation logic | `ChainingPeakCondition`, `ThresholdCondition` |
| `*Policy` | Behavioral strategy | `InsertionPolicy`, `SingleStageChainingPolicy` |
| `*Stage` | Processing unit | `CutStage`, `BackgroundStage`, `FilterStage` |
| `*Metadata` | Data container | `FlowMetadata`, `StageMetadata`, `SampleMetadata` |
| `*Request` | Request object | `StageRequest`, `StageApprovalRequest` |
| `*Scheduler` | Scheduling logic | `BaseScheduler`, `FIFOScheduler` |
...
**Rule:** Avoid abbreviations unless they improve readability significantly. When abbreviating, be consistent.
...
**Decision:** For SAXS project, **prefer full words** going forward. Existing `abst_` files may remain but new files should use full `abstract_`.
...
- Single trailing underscore `_` (reserved for avoiding keyword conflicts)
...
|--------|---------|---------|
| `get_*` | Retrieve value | `get_metadata()`, `get_cut_point()` |
| `set_*` | Set value (rare, prefer immutability) | `set_metadata()` |
| `create_*` | Factory method | `create_sample()`, `create_request()` |
| `is_*` | Boolean check | `is_valid()`, `is_processed()` |
| `has_*` | Boolean check (possession) | `has_metadata()`, `has_peaks()` |
| `*_from_*` | Conversion | `from_dict()`, `from_yaml()` |
...
original_intensity_values_from_saxs_sample = sample.get_intensity()
```

**Guidelines:**
- 2-3 words maximum for variable names
- Use full words, not abbreviations (except standard ones like `q`, `I`, `dI` for SAXS data)
...
**Rule:** All public functions, methods, and class attributes must have type annotations.
...
**Rule:** Use string quotes for forward references to avoid circular imports.
...
**Rule:** Break complex type annotations across multiple lines for readability.
...
# ❌ INCORRECT (too long)
def create_pipeline(stages: list[IAbstractStage[Any]], metadata: dict[str, Any], config: PipelineConfig | None = None) -> tuple[Pipeline, FlowMetadata]:
    """Create pipeline."""
```
...
**Rule:** Use `TypedDict` for structured dictionaries, especially for schemas.
...
Extended description explaining the purpose, behavior, and usage
of this module/class/function. Can span multiple paragraphs.
...
**Rule:** Document all public methods with Parameters, Returns, and Raises sections.
...
- ✅ **Public method docstrings:** Always required with Parameters/Returns
...
**Rule:** Group imports in the following order with blank lines between groups:
...
```python
# ✅ CORRECT
# SAXS data below q=0.02 is unreliable due to beamstop artifacts
cutoff_index = np.argmax(q > 0.02)

# Use BFGS optimizer for better convergence on smooth functions
...
**Rule:** Use comment separators for logical sections within long functions/classes.
...
```python
# ✅ CORRECT
logger.info("Processing sample %s with %d peaks", sample_id, peak_count)
logger.debug("Metadata: %s", metadata)

# ❌ INCORRECT
logger.info(f"Processing sample {sample_id} with {peak_count} peaks")
```

**Rationale:** Lazy evaluation - string formatting only happens if log level is enabled.
...
**Rationale:** Easier to debug; message variable visible in stack trace.
...
# ❌ INCORRECT
message = "Processing {} with {} peaks".format(sample_id, peak_count)
message = "Processing %s with %d peaks" % (sample_id, peak_count)
```
...
class Pipeline:
    @classmethod
    def with_stages(cls, stages: list[IAbstractStage]) -> "Pipeline":
...
# ❌ INCORRECT (>88 chars)
def process_sample(sample: SAXSSample, metadata: StageMetadata | None = None) -> tuple[SAXSSample, FlowMetadata]:
```
